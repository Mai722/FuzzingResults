...
results <- read.csv("C:/CSV/results.csv")
...
shapes <- c(1,2,0,5,6,7,12,8,9,20,19,18,17,16,15)

dfcode=results[which(results$SUT=='jpegtran'),c(1,2,3,4,8)]
...
p <- ggplot(dfdata,aes(x=Time, y=Bugs, shape=as.factor(Repetition)))+
  geom_line( size=0.5)+ 
  geom_point(size = 4)+(scale_shape_manual(values = shapes, name="Repetition"))+
