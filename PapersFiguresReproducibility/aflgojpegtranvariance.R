...
aflgovariancedif <- read.csv2("C:/Users/aflgovariancedif.csv")

aflgovariancedif <- cbind(aflgovariancedif,Code=c("jpegtran"))


p <- ggplot(aflgovariancedif,aes(x=aflgovariancedif$Dif.varianzas, y=aflgovariancedif$jpegtran))+
  geom_line( size=0.5)+ 
...
p + ggtitle("AFLgo fuzzing results") + labs(y=expression(rho), x = "Number of repetitions")
