...
aflfastvariancedif <- read.csv2("C:/Users/aflfastvariancedif.csv")

aflfastvariancedif <- cbind(aflfastvariancedif,Code=c("pdftops"))


p <- ggplot(aflfastvariancedif,aes(x=aflfastvariancedif$Dif.varianzas, y=aflfastvariancedif$pdftops))+
  geom_line( size=0.5)+ 
...
p + ggtitle("AFLfast fuzzing results") + labs(y=expression(rho), x = "Number of repetitions")
